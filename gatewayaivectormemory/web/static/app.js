...
const api = (path, opts = {}) => {
  ...
  const token = localStorage.getItem('tavm_token');
  ...
  return fetch(url, {
    ...
    if (r.status === 401 || r.status === 403) { showLoginOverlay(); return Promise.reject(new Error('Unauthorized')); }
    return r.json();
  });
};
...
function renderMemoryCard(m) {
  ...
  const scopeCls = { project: 'blue', user: 'cyan', team: 'purple' };
  const scopeBadge = m._scope ? `<span class="badge badge--${scopeCls[m._scope] || 'muted'} badge--xs">${t('scope_' + m._scope) || m._scope}</span>` : '';
  const createdBy = m._scope === 'team' && m.created_by ? `<span class="memory-card__created-by">${t('createdBy')} ${escHtml(m.created_by)}</span>` : '';
  return `<div class="memory-card" style="cursor:pointer" onclick="if(!event.target.closest('.memory-card__actions')){editMemory('${m.id}')}">
    <div class="memory-card__header">
      <div class="memory-card__header-left">
        <div class="memory-card__id">${m.id}${scopeBadge ? ' ' + scopeBadge : ''}</div>
        <div class="memory-card__tags">${tags.map(tg => `<span class="tag">${escHtml(TAG_I18N[tg] ? t(TAG_I18N[tg]) : tg)}</span>`).join('')}</div>
        <div class="memory-card__time">${formatTime(m.created_at)}${createdBy ? ' · ' + createdBy : ''}</div>
      </div>
...
function loadProjectMemories() {
  const scope = $('#project-scope-filter')?.value || 'project';
  loadMemoriesByScope(scope, '#project-memory-list', '#project-pager', '#project-search', '#project-count', 'projectPage');
}
$('#project-scope-filter')?.addEventListener('change', () => { state.projectPage = 1; loadProjectMemories(); });
...
async function loadStats() {
  ...
  const cards = [
    ...
    { label: t('teamMemories'), num: mem.team || 0, cls: 'purple', action: 'goto-tab', tab: 'project-memories' },
  ...
  const totalCount = (mem.project || 0) + (mem.team || 0) + (mem.user || 0) + (issues.pending || 0) + (issues.in_progress || 0) + (issues.completed || 0);
...
function renderTagTable() {
  ...
  tbody.innerHTML = tagData.map(tg => `
    <tr data-tag="${escHtml(tg.name)}">
      ...
      <td><span class="tag-count">${tg.project_count ? `${tg.project_count} 📁` : ''}${tg.project_count && (tg.team_count || tg.user_count) ? '  ' : ''}${tg.team_count ? `${tg.team_count} 👥` : ''}${tg.team_count && tg.user_count ? '  ' : ''}${tg.user_count ? `${tg.user_count} 🌐` : ''}</span></td>
...
function loadProjects() {
  fetch('/api/projects').then(r => r.json()).then(data => {
    ...
    $('#project-select-footer').innerHTML = `${t('footer').replace('{n}', data.projects.length)} · <a href="https://github.com/cmsyt/gatewayaivectormemory" target="_blank" rel="noopener" style="color:#6366F1;text-decoration:none">GitHub</a>`;
...
$('#btn-export')?.addEventListener('click', async () => {
  ...
  a.download = `gatewayaivectormemory-export-${new Date().toISOString().slice(0, 10)}.json`;
  a.click();
...
function hideLoginOverlay() {
  ...
  const logoutBtn = $('#btn-logout');
  if (logoutBtn && localStorage.getItem('tavm_token')) logoutBtn.style.display = '';
}

$('#login-submit')?.addEventListener('click', async () => {
  const token = $('#login-token-input').value.trim();
  if (!token) return;
  const errEl = $('#login-error');
  errEl.classList.add('hidden');
  localStorage.setItem('tavm_token', token);
  try {
    const r = await fetch('/api/health', { headers: { 'Authorization': `Bearer ${token}` } });
    if (r.status === 401 || r.status === 403) {
      localStorage.removeItem('tavm_token');
...
$('#login-token-input')?.addEventListener('keydown', (e) => {
...
async function checkAuth() {
  try {
    ...
    if (r.status === 401 || r.status === 403) { showLoginOverlay(); return false; }
    const logoutBtn = $('#btn-logout');
    if (logoutBtn && token) logoutBtn.style.display = '';
    return true;
  } catch { return true; }
}
